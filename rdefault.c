...
char *resolver_default_stack_asm_address(int stack_offset, char *out) {
  if (stack_offset < 0) {
    sprintf(out, "ebp%d", stack_offset);
    return out;
  }
...
struct resolver_default_entity_data *resolver_default_new_entity_data() {
  struct resolver_default_entity_data *entity_data =
      calloc(1, sizeof(struct resolver_default_entity_data));
  return entity_data;
}
...
void resolver_default_global_asm_address(const char *name, int offset,
  ...
  sprintf(address_out, "%s+%d", name, offset);
}
...
resolver_default_new_scope_entity(struct resolver_process *process,
  ...
  struct resolver_default_entity_data *entity_data =
      resolver_default_new_entity_for_var_node(variable_node(var_node), offset,
                                               flags);
  return resolver_new_entity_for_var_node(process, variable_node(var_node),
                                          entity_data, offset);
}
