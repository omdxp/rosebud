...
#define STRUCT_PUSH_START_POSITION_ONE 1
...
enum {
  ...
  CODEGEN_ENTITY_RULE_IS_WILL_PEEK_AT_EBX = 0b00001000,
};
...
void codegen_generate_struct_push(struct resolver_entity *entity,
                                  struct history *history, int start_pos);
void codegen_plus_or_minus_string_for_value(char *out, int value, size_t size);
...
void codegen_begin_switch_statement() {
  struct code_generator *gen = current_process->generator;
  struct generator_switch_stmt *switch_stmt_data = &gen->_switch;
  ...
  asm_push(".switch_stmt_%d:", switch_stmt_id);
...
void codegen_end_switch_statement() {
  struct code_generator *gen = current_process->generator;
  struct generator_switch_stmt *switch_stmt_data = &gen->_switch;
  asm_push(".switch_stmt_%d_end:", switch_stmt_data->current.id);
  // restore older switch statement
...
void codegen_begin_case_statement(int index) {
  struct code_generator *gen = current_process->generator;
  struct generator_switch_stmt *switch_stmt_data = &gen->_switch;
  asm_push(".switch_stmt_%d_case_%d:", switch_stmt_data->current.id, index);
}
...
void codegen_generate_global_variable_for_struct(struct node *node) {
  if (node->var.val != NULL) {
    compiler_error(current_process, "Structs with values not supported");
  }
...
void codegen_generate_struct_push_or_return(struct resolver_entity *entity,
                                            struct history *history,
                                            int start_pos) {
  codegen_generate_struct_push(entity, history, start_pos);
}
...
void codegen_gen_mem_access(struct node *node, int flags,
  ...
  if (datatype_is_struct_or_union_no_pointer(&entity->dtype)) {
    ...
    asm_push_ins_pop("ebx", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                     "result_value");
    codegen_generate_struct_push_or_return(entity, history_begin(0), 0);
...
void codegen_generate_unary_address(struct node *node,
  ...
  codegen_response_acknowledge(
      RESPONSE_SET(.flags = RESPONSE_FLAG_UNARY_GET_ADDRESS));
}
...
void codegen_generate_unary_indirection(struct node *node,
  ...
  codegen_generate_expressionable(
      ...
      history_down(history,
                   flags | EXPRESSION_GET_ADDRESS | EXPRESSION_GET_ADDRESS));
...
void codegen_generate_unary_node(struct node *node, struct history *history) {
  int flags = history->flags;
...
void codegen_generate_tenary_node(struct node *node, struct history *history) {
  ...
  asm_push_ins_pop("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                   "result_value");
  asm_push("cmp eax, 0");
  asm_push("je .tenary_false_%d", false_label_id);
  asm_push(".tenary_true_%d:", true_label_id);

  codegen_generate_expressionable(node->tenary.true_node,
                                  history_down(history, 0));
  asm_push_ins_pop_or_ignore("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                             "result_value");
  asm_push("jmp .tenary_end_%d", end_label_id);

  asm_push(".tenary_false_%d:", false_label_id);
  codegen_generate_expressionable(node->tenary.false_node,
                                  history_down(history, 0));
  asm_push_ins_pop_or_ignore("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                             "result_value");
...
int codegen_entity_rules(struct resolver_entity *last_entity,
  ...
  if (datatype_is_struct_or_union_no_pointer(&last_entity->dtype)) {
    rule_flags |= CODEGEN_ENTITY_RULE_IS_STRUCT_OR_UNION_NO_POINTER;
  }
  ...
  if (last_entity->type == RESOLVER_ENTITY_TYPE_FUNCTION_CALL) {
    ...
    rule_flags |= CODEGEN_ENTITY_RULE_IS_WILL_PEEK_AT_EBX;
  }
...
void codegen_generate_entity_access_for_unary_indirection_for_assignment_left_operand(
    struct resolver_result *result, struct resolver_entity *entity,
    struct history *history) {
  asm_push("; unary indirection for assignment left operand");
...
void codegen_generate_mov_struct(struct datatype *dtype,
                                 const char *base_address, off_t offset) {
  size_t struct_size = align_value(datatype_size(dtype), DATA_SIZE_DWORD);
  int pops = struct_size / DATA_SIZE_DWORD;
...
void codegen_generate_assignment_part(struct node *node, const char *op,
  ...
  if (!next_entity) {
    if (datatype_is_struct_or_union_no_pointer(&result->last_entity->dtype)) {
      codegen_generate_mov_struct(&result->last_entity->dtype,
...
void codegen_generate_entity_access_for_function_call(
  ...
  if (datatype_is_struct_or_union_no_pointer(&entity->dtype)) {
    ...
    codegen_generate_struct_push(entity, history_begin(0), 0);
  } else {
    asm_push_ins_push_with_data(
        "eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE, "result_value", 0,
        &(struct stack_frame_data){.dtype = entity->dtype});
  ...
  struct resolver_entity *next_entity = resolver_entity_next(entity);
  if (next_entity && datatype_is_struct_or_union(&next_entity->dtype)) {
    asm_push_ins_pop("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                     "result_value");
    asm_push("mov ebx, eax");
...
void codegen_generate_struct_push(struct resolver_entity *entity,
                                  struct history *history, int start_pos) {
  asm_push("; struct push");
  size_t struct_size = align_value(entity->dtype.size, DATA_SIZE_DWORD);
  int pushes = struct_size / DATA_SIZE_DWORD;
...
void codegen_generate_else_or_else_if(struct node *node, int end_label_id) {
  if (node->type == NODE_TYPE_STATEMENT_IF) {
    ...
    compiler_error(current_process, "Unknown node type");
  }
}
...
void _codegen_generate_if_statement(struct node *node, int end_label_id) {
  ...
  asm_push_ins_pop("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                   "result_value");
  asm_push("cmp eax, 0");
  asm_push("je .if_%d", if_label_id);
  codegen_generate_body(node->stmt.if_stmt.body_node,
                        history_begin(IS_ALONE_STATEMENT));
  asm_push("jmp .if_end_%d", end_label_id);
  asm_push(".if_%d:", if_label_id);
...
void codegen_generate_do_while_statement(struct node *node) {
  ...
  asm_push(".do_while_start_%d:", do_while_start_id);
  codegen_generate_body(node->stmt.do_while_stmt.body,
                        history_begin(IS_ALONE_STATEMENT));
  codegen_generate_expressionable(node->stmt.do_while_stmt.cond,
                                  history_begin(0));
  asm_push_ins_pop("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                   "result_value");
  asm_push("cmp eax, 0");
  asm_push("jne .do_while_start_%d", do_while_start_id);
  codegen_end_entry_exit_point();
}
...
void codegen_generate_if_statement(struct node *node) {
  int end_label_id = codegen_label_count();
  _codegen_generate_if_statement(node, end_label_id);
  asm_push(".if_end_%d:", end_label_id);
}
...
void codegen_generate_for_statement(struct node *node) {
  ...
  asm_push(".for_start_%d:", for_start_id);
  if (for_stmt->cond) {
    codegen_generate_expressionable(for_stmt->cond, history_begin(0));
    asm_push_ins_pop_or_ignore("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                               "result_value");
    asm_push("cmp eax, 0");
    asm_push("je .for_end_%d", for_end_id);
  }

  if (for_stmt->body) {
    codegen_generate_body(for_stmt->body, history_begin(IS_ALONE_STATEMENT));
  }
  ...
  asm_push("jmp .for_start_%d", for_start_id);
  asm_push(".for_end_%d:", for_end_id);
...
void codegen_generate_switch_default_stmt(struct node *node) {
  ...
  struct code_generator *gen = current_process->generator;
  struct generator_switch_stmt *switch_stmt_data = &gen->_switch;
  asm_push(".switch_stmt_%d_case_default:", switch_stmt_data->current.id);
}
...
void codegen_generate_switch_stmt_case_jumps(struct node *node) {
  ...
  while (switch_case) {
    asm_push("cmp eax, %d", switch_case->index);
    asm_push("je .switch_stmt_%d_case_%d", codegen_switch_id(),
             switch_case->index);
    switch_case = vector_peek(node->stmt.switch_stmt.cases);
  }

  if (node->stmt.switch_stmt.has_default_case) {
    asm_push("jmp .switch_stmt_%d_case_default", codegen_switch_id());
    return;
  }
...
void codegen_generate_switch_case_statement(struct node *node) {
  ...
  asm_push("; case %d", case_stmt_exp->llnum);
